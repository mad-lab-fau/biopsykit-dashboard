...
def test_set_sleep_data_parameters_constructor(set_sleep_data_parameters):
    assert set_sleep_data_parameters is not None


def test_set_sleep_data_parameters_show_parameters(set_sleep_data_parameters):
    ...
    col = set_sleep_data_parameters.get_parameter_column_for_selected_device()
    assert len(col.objects) > 0
    assert valid_parameters_are_shown(col, "Polysomnography", set_sleep_data_parameters)
    set_sleep_data_parameters.selected_device = "Withings"
    col = set_sleep_data_parameters.get_parameter_column_for_selected_device()
    assert len(col.objects) > 0
    assert valid_parameters_are_shown(col, "Withings", set_sleep_data_parameters)


def test_set_sleep_parameters_select_all_parameters(set_sleep_data_parameters):
    ...
    for device in possible_devices:
        ...
        for input_widget in set_sleep_data_parameters.parameter_column.objects:
            if isinstance(input_widget, pn.widgets.Select):
                ...
                assert (
                    set_sleep_data_parameters.selected_parameters["Polysomnography"][
                        input_widget.name
                    ]
