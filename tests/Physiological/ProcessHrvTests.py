...
@pytest.fixture
def ask_to_process_hrv():
    return AskToProcessHRV()


@pytest.fixture
def set_hrv_parameters():
    return SetHRVParameters()


def test_constructor(ask_to_process_hrv):
    assert ask_to_process_hrv.ready == False


def test_btn_behavior(ask_to_process_hrv):
...
def test_set_hrv_parameter_change_values(set_hrv_parameters):
